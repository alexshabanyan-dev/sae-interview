      ...
      <Stack gap="xl">
        ...
        <Stack gap="xs">
          ...
          <Text size="lg" c="dimmed" maw={640}>
            Максимально подробно: лексические окружения, связь с TDZ и циклами,
            память, отличие от «просто передать аргумент», практика и задачи.
          </Text>
        </Stack>
...
`}</CodeBlock>
        ...
        <JsTopicSection title="10. Три уровня одной картины: запись, ссылка, GC">
          <List spacing="sm" c="gray.3" size="sm">
            <List.Item>
              <strong>Лексическое окружение (Environment Record)</strong> — где
              физически живут привязки переменных для блока/функции. Пока выполняется
              код, движок создаёт и связывает записи.
            </List.Item>
            <List.Item>
              <strong>Функция как объект</strong> хранит ссылку на внешнее
              окружение (в учебной модели — <Code>[[Environment]]</Code>). Когда
              внутреннюю функцию возвращают или сохраняют, эта ссылка не
              «отрезается»: внешнее окружение остаётся достижимым.
            </List.Item>
            <List.Item>
              <strong>Сборщик мусора</strong> не трогает окружение, пока на него
              есть путь от живой функции. Как только последняя функция с этим
              замыканием недостижима — вся связанная цепочка может быть собрана.
            </List.Item>
          </List>
          <Text c="gray.2" lh={1.75} mt="sm">
            Запомни формулу: <strong>замыкание = функция + живое внешнее
            окружение</strong>, к которому у неё есть доступ при каждом вызове.
          </Text>
        </JsTopicSection>

        <JsTopicSection title="11. TDZ, let в цикле и отдельные окружения">
          <Text c="gray.2" lh={1.75} mb="sm">
            У <Code>let</Code>/<Code>const</Code> есть <strong>временная мёртвая
            зона</strong> (TDZ): до объявления переменную нельзя читать. В цикле{" "}
            <Code>for (let i = 0; …)</Code> на <strong>каждой итерации</strong> для
            тела цикла создаётся своё блочное окружение со своим <Code>i</Code> —
            поэтому стрелка или функция, созданная внутри итерации, замыкается на{" "}
            <em>свой</em> <Code>i</Code>, а не на одну общую переменную как у{" "}
            <Code>var</Code>.
          </Text>
          <CodeBlock>{`// var: одна i на весь цикл → колбэки видят финальное значение
// let: своя i на каждую итерацию → каждый колбэк замыкается на своё значение`}</CodeBlock>
          <Text c="gray.3" size="sm" mt="sm">
            Понимание TDZ помогает не путать «ошибка до объявления» с «не то
            значение в замыкании».
          </Text>
        </JsTopicSection>

        <JsTopicSection title="12. Замыкание или просто аргумент?">
          <Text c="gray.2" lh={1.75} mb="sm">
            Не всё, что «видит снаружу», требует замыкания в смысле собеса.
          </Text>
          <List spacing="sm" c="gray.3" size="sm">
            <List.Item>
              Если достаточно передать <strong>текущее значение</strong> примитива
              в функцию как параметр — замыкание на изменяемое состояние не
              нужно: <Code>{"arr.forEach((_, i) => …)"}</Code> получает <Code>i</Code>{" "}
              аргументом.
            </List.Item>
            <List.Item>
              Замыкание <strong>нужно</strong>, когда функция должна многократно
              обращаться к <strong>одному и тому же скрытому состоянию</strong>{" "}
              (счётчик, кеш, приватное поле фабрики), которое живёт между вызовами.
            </List.Item>
            <List.Item>
              Если замкнуть <strong>изменяемый объект</strong>, все вызовы видят
              его актуальные поля — это не «снимок на момент создания», если сам не
              скопировал данные в примитивы/новый объект.
            </List.Item>
          </List>
        </JsTopicSection>

        <JsTopicSection title="13. Чеклист типичных утечек через замыкание">
          <List spacing="sm" c="gray.3" size="sm">
            <List.Item>
              Глобальный массив/карта, куда бесконечно пушатся колбэки или
              «обработчики» — каждый держит окружение.
            </List.Item>
            <List.Item>
              <Code>addEventListener</Code> без соответствующего{" "}
              <Code>removeEventListener</Code>, если обработчик замыкает большой
              объект.
            </List.Item>
            <List.Item>
              Долгоживущий таймер/интервал, в замыкании которого лежит весь
              предыдущий контекст страницы или тяжёлый DOM.
            </List.Item>
            <List.Item>
              Кеш без лимита размера (Map в замыкании модуля) — не утечка в смысле
              GC «навсегда», но рост памяти до OOM.
            </List.Item>
          </List>
          <Alert color="orange" variant="light" mt="md">
            <Text size="sm" lh={1.65}>
              Лечение: обнулять ссылки, снимать слушатели, ограничивать кеш,
              выносить тяжёлое в WeakMap/слабые ссылки где уместно, не хранить в
              замыкании больше данных, чем нужно для работы API.
            </Text>
          </Alert>
        </JsTopicSection>

        <JsTopicSection title="14. Частые вопросы на интервью">
          <Table
            striped
            highlightOnHover
            withTableBorder
        ...
        <JsTopicFoldableSection
          title="15. Задачи для практики (50 шт.)"
          hint={
        ...
        <JsTopicSection title="16. Шпаргалка одним абзацем">
          <Paper
            p="lg"
            radius="md"
            withBorder
